			...
			<table class="table no-margin table-striped table-bordered">
				<thead>
					<tr>
						...
						<g:sortableColumn property="geolocation" title="${message(code: 'scheduledCourse.geolocation.label', default: 'Geolocalizacion')}" />

						<g:sortableColumn property="fullAddress" title="${message(code: 'scheduledCourse.fullAddress.label', default: 'Dirección')}" />
				...
				<tbody>
					...
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:formatDate format="dd-MM-yyyy" date="${scheduledCourseInstance.beginDate}" /></td>

						<td><g:formatDate format="dd-MM-yyyy" date="${scheduledCourseInstance.limitRegistrationDate}" /></td>

						<td>${fieldValue(bean: scheduledCourseInstance, field: "geolocation")}</td>

						<td>${scheduledCourseInstance.fullAddress}</td>
